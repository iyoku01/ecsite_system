...
<body>
    ...
    <div class="revProduct clearfix">
        ...
        <div class="innerElement clearfix">
            <img class=float src="${product.mainPic_file}" width="80">
        ...
        <c:if test="${writeReview}">
            <form action="reviewWrite.jsp" method="post">
                ...
                <input type=hidden name=review value="${review.review}"> <input
    ...
    <div class=reviewList>
        ...
        <c:if test="${!empty writeReview}">
            <c:choose>
                <c:when test="${!empty review}">
                    ...
                    <div class="ownReview">
                        <span class="mozifont">投稿者</span> <span
                            class="mozifont reviewList_bluecolor">${review.nickname}</span> <span
                            class="mozifont"><fmt:formatDate value="${review.date}"
                        ...
                        <c:forEach begin="1" end="5" step="1" varStatus="status">
                            <c:choose>
                                <c:when test="${status.index <= review.evaluation}">
                                    <yellowStar>&#xf2fc;</yellowStar>
                                </c:when>
                                <c:otherwise>
                                    <grayStar>&#xf2fc;</grayStar>
        ...
        <c:forEach var="i" items="${reviewList}">
            <div class="oneReview">
                <span class="mozifont">投稿者</span> <span
                    class="mozifont reviewList_bluecolor">${i.nickname}</span> <span
                    class="mozifont"><fmt:formatDate value="${i.date}"
                        type="DATE" dateStyle="FULL" /></span>
                <c:forEach begin="1" end="5" step="1" varStatus="status">
                    <c:choose>
                        <c:when test="${status.index <= i.evaluation}">
                            <yellowStar>&#xf2fc;</yellowStar>
                        </c:when>
                        <c:otherwise>
                            <grayStar>&#xf2fc;</grayStar>
